...
LogiKeyMapSim の current release-driven development で使用する Operating Model を定義します。
...
- Node/Next.js/React/package.json が application toolchain authority。Bun は Agent Skills CLI 用 auxiliary tool としてのみ導入する。
- keyboard mapping semantics / physical-key vs logical-layout transformation を product domain authority として UI convenience から推測して変更しない。
- browser interaction / keyboard-event verification と source-only lint/test/build evidence を分離する。
